    ...
    mclOutputHandlerFcn print_handler)
{
    ...
    if (!mclmcrInitialize())
        return false;
    if (!GetModuleFileName(GetModuleHandle("t_Product"), path_to_dll, _MAX_PATH))
        return false;
...
{
    return t_ProductInitializeWithHandlers(mclDefaultErrorHandler, 
...
{
    return mclFeval(_mcr_inst, "t_Product", nlhs, plhs, nrhs, prhs);
}

LIB_t_Product_CPP_API 
void MW_CALL_CONV t_Product(int nargout, mwArray& C, const mwArray& A, const mwArray& B)
{
    mclcppMlfFeval(_mcr_inst, "t_Product", nargout, 1, 2, &C, &A, &B);
}
