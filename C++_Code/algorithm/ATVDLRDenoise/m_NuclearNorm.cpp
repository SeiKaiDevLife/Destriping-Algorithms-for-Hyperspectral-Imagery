...
#define EXPORTING_m_NuclearNorm 1
    ...
    mclOutputHandlerFcn print_handler)
{
    ...
    if (!mclmcrInitialize())
        return false;
    if (!GetModuleFileName(GetModuleHandle("m_NuclearNorm"), path_to_dll, _MAX_PATH))
        return false;
...
{
    return m_NuclearNormInitializeWithHandlers(mclDefaultErrorHandler, 
...
{
    return mclFeval(_mcr_inst, "m_NuclearNorm", nlhs, plhs, nrhs, prhs);
}

LIB_m_NuclearNorm_CPP_API 
void MW_CALL_CONV m_NuclearNorm(int nargout, mwArray& Result, const mwArray& X)
{
    mclcppMlfFeval(_mcr_inst, "m_NuclearNorm", nargout, 1, 1, &Result, &X);
}
